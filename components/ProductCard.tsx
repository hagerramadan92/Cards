...
export default function ProductCard({
  ...
  // ✅ تحديث منطق عرض الخصم
  const showDiscountChip = useMemo(() => {
    // تحقق من وجود خصم فعلي
    if (!product?.has_discount) return false;
    if (!product?.discount) return false;
    
    const discountValue = product.discount.value;
    
    // تأكد من أن قيمة الخصم موجودة وليست صفر
    return discountValue && Number(discountValue) > 0;
  }, [product]);

  const getDiscountText = useMemo(() => {
    if (!showDiscountChip) return '';
