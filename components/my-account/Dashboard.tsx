...
const LEVELS: LevelCard[] = [
	{
		...
		features: [
			'level.feature.blue.lucky_code',
			'level.feature.blue.standard_support'
		],
		isLocked: true,
	},
	{
		...
		features: [
			'level.feature.silver.lucky_codes',
			'level.feature.silver.weekly_lucky_code',
			'level.feature.silver.extra_points_5',
			'level.feature.silver.special_prices_3',
			'level.feature.silver.standard_support'
		],
		isLocked: true,
	},
	{
		...
		features: [
			'level.feature.gold.lucky_codes_5',
			'level.feature.gold.daily_lucky_code',
			'level.feature.gold.extra_points_10',
			'level.feature.gold.special_prices_5',
			'level.feature.gold.premium_support'
		],
		isLocked: true,
	},
	{
		...
		features: [
			'level.feature.titanium.lucky_codes_7',
			'level.feature.titanium.daily_lucky_codes_2',
			'level.feature.titanium.extra_points_15',
			'level.feature.titanium.special_prices_10',
			'level.feature.titanium.priority_support'
		],
		isLocked: true,
	},
	{
		...
		features: [
			'level.feature.diamond.lucky_codes_10',
			'level.feature.diamond.daily_lucky_codes_3',
			'level.feature.diamond.extra_points_20',
			'level.feature.diamond.special_prices_20',
			'level.feature.diamond.whatsapp_support',
			'level.feature.diamond.premium_account',
			'level.feature.diamond.free_like_card',
			'level.feature.diamond.iphone_prize'
		],
		isLocked: true,
	},
];
							...
							<div className="sticky top-0 bg-white border-b border-slate-200 px-3 sm:px-6 py-3 sm:py-4 z-20">
								<div className="flex items-start justify-between gap-2 sm:gap-4">
									<div className="flex-1">
										<h2 className="text-lg sm:text-xl md:text-2xl font-bold text-slate-900 mb-1 sm:mb-2">
											{t('level_modal.title')}
										</h2>
										<p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
											{t('level_modal.description')}
										</p>
									</div>
									<button
										onClick={onClose}
										...
										<div key={index} className="flex flex-col items-center flex-shrink-0 mx-2">
											{level.isCurrent && (
												<div className="bg-pro text-white text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl rounded-br-none rounded-bl-none flex items-center gap-1 sm:gap-1.5">
													...
													<span>{t('level_modal.current_level')}</span>
												</div>
											)}
											...
											{level.isLocked && (
												...
												>
													...
													<span>{t('level_modal.locked')}</span>
												</div>
											)}
											...
											>
												...
												{level.isCurrent && (
													<div className={`text-start my-1 sm:my-2 ${level.textColor}`}>
														<p className="text-[10px] sm:text-xs leading-relaxed">
															{t('level_modal.start_blue_message')}
														</p>
												...
												{level.purchaseAmount && (
													<div className={`text-start my-1 sm:my-2 ${level.textColor}`}>
														<p className="text-[10px] sm:text-xs leading-relaxed">
															{t('level_modal.next_level_message', { 
																level: t(level.name  as any), 
																amount: level.purchaseAmount 
															})}
														</p>
														<div className="w-full h-px bg-white/20 mt-2 sm:mt-3"></div>
													</div>
												...
												<div className="space-y-2 sm:space-y-3 ps-0 ms-0">
													{level.features.map((featureKey, idx) => (
														<div key={idx} className={`flex gap-1 sm:gap-1.5 ${level.textColor} text-[10px] sm:text-xs text-start`}>
															...
															<span className="leading-relaxed">{t(featureKey as any)}</span>
														</div>
													))}
												</div>
											</div>
			...
			<div className='bg-white rounded-xl pb-3 border border-slate-200 shadow-sm overflow-hidden'>
				<div className='px-4 py-3 md:px-5 md:py-4 border-b border-slate-100'>
					<div className='flex items-center justify-between mb-3 md:mb-4'>
						<h2 className='text-sm md:text-base font-semibold text-slate-500'>{t('finance.like_card_balance')}</h2>
						<Link 
							...
							{walletData && (
								<div className='mt-3 text-xs text-slate-500'>
									<div className='flex justify-between'>
										<span>{t('finance.daily_limit')}:</span>
										<span className='font-medium'>{walletData.daily_limit} {currency}</span>
									</div>
									<div className='flex justify-between mt-1'>
										<span>{t('finance.total_deposits_today')}:</span>
				...
				>
					...
					>
						...
						<span className='text-xs md:text-sm font-semibold text-pro-max group-hover:text-pro-max transition-colors'>{t('finance.vouchers')}</span>
					</Link>
					...
					>
						...
						<span className='text-xs md:text-sm font-semibold text-pro-max group-hover:text-pro-max transition-colors'>{t('finance.charge_balance')}</span>
					</Link>
					...
					>
						...
						<span className='text-xs md:text-sm font-semibold text-pro-max group-hover:text-pro-max transition-colors'>{t('finance.payment_link')}</span>
					</Link>
				</div>
			</div>
		</div>
			...
			{/* <div className='bg-white border border-slate-200 rounded-xl shadow-sm'>
				...
				<div className='relative gradient-blue-background text-white rounded-lg flex flex-col gap-2 py-3 md:py-4 mx-3 md:mx-4 mb-3 md:mb-4'>
					<div className='flex items-end gap-0.5 px-3 md:px-6'>
						...
						<span className='text-xs md:text-base'>{t('dashboard.points')}</span>
					</div>
					...
					<div className='flex items-center px-3 md:px-6 gap-2'>
						<div className='flex items-center gap-0.5 mt-1 md:mt-2 bg-white rounded-full px-1 py-0.5 w-fit'>
							...
							/>
							<p className='text-[10px] md:text-xs text-pro font-bold me-1'>{t('dashboard.blue_level')}</p>
						</div>
						<button
						...
						>
							<p className='text-[10px] md:text-sm'>{t('dashboard.discover_features')}</p>
					...
					<div className='flex items-center gap-0.5 px-3 md:px-6 mt-1'>
						<p className='text-[10px] md:text-sm opacity-50 me-1 line-clamp-1'>
							{t('dashboard.buy_for_level_up')}
						</p>
						<button
							onClick={() => setShowLevelModal(true)}
							className='flex items-center md:gap-0.5 hover:opacity-80 transition-opacity cursor-pointer flex-shrink-0'
						>
							<p className='text-[10px] md:text-sm font-bold whitespace-nowrap'>{t('dashboard.silver_level')}</p>
							<MdKeyboardArrowLeft size={18} className='hidden sm:block'/>
						</button>
					</div>
