...
async function getWalletData(): Promise<WalletData | null> {
  try {
    // في Server Components، يمكننا استخدام cookies مباشرة
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/user/wallet`, {
      ...
      headers: {
        ...
        'Accept-Language': `${navigator.language || 'en-US'}`
         // إرسال اللغة الحالية للمستخدم,
        // إذا كان التوكن في cookies، سيرسلها تلقائياً
      },
